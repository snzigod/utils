...
public class AzDG {
	...
	public static byte[] encrypt(byte txt[], byte key[]) {
		...
		new Md5();
		byte encrypt_key[] = Md5
				.getMd5ByString(
						(new StringBuilder(String.valueOf(rand))).toString())
		...
		return getBASE64(new String(encodeKey(byteOut.toByteArray(), key)))
				.getBytes();
	}
	...
	public static String decrypt(String txt) {
		return new String(
				decrypt(txt.getBytes(), PASSWORD_CRYPT_KEY.getBytes()));
	}

	public static String encrypt(String txt) {
		return new String(
				encrypt(txt.getBytes(), PASSWORD_CRYPT_KEY.getBytes()));
	}

	public static byte[] encodeKey(byte txt[], byte encrypt_key[]) {
		new Md5();
		encrypt_key = Md5.getMd5ByString(new String(encrypt_key)).toLowerCase()
				.getBytes();
		byte ctr = 0;
