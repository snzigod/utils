...
public class ZXingCodeUtil {
	...
	private static final int BGWHITE = 0xFFFFFFFF; // 背景默认白色

	public static void main(String[] args) throws WriterException {
		try {
			String filePath = "e:\\Users\\snzig\\Pictures\\psb.jpg"; // TODO
			File logoFile = new File(filePath);
			getLogoQRCode("https://www.baidu.com/", logoFile, "跳转到百度的二维码");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @return
	 */
	public static String getLogoQRCode(String content, File logoFile,
			String qrName) {
		try {
			ZXingCodeUtil zx = new ZXingCodeUtil();
			BufferedImage bim = zx.getQR_CODEBufferedImage(content,
					BarcodeFormat.QR_CODE, 400, 400, zx.getDecodeHintType());
			return zx.addLogo_QRCode(bim, logoFile, new LogoConfig(), qrName,
					"d:/");
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	 ...
	 * @return
	 */
	public String addLogo_QRCode(BufferedImage image, File logoPic,
			LogoConfig logoConfig, String qrName, String qrCodePath) {
		try {
			/** * 读取二维码图片，并构建绘图对象 */
			if (logoPic != null) {
			...
			if (StringUtil.isNotEmpty(qrName)) {
				...
				int strWidth = outg.getFontMetrics().stringWidth(qrName);
				if (strWidth > 399) {
					// //长度过长就截取前面部分
					// outg.drawString(productName, 0, image.getHeight() +
					// (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
					// 长度过长就换行
					String productName1 = qrName.substring(0,
							qrName.length() / 2);
					String productName2 = qrName.substring(qrName.length() / 2,
							qrName.length());
			...
			if (StringUtil.isNotEmpty(qrCodePath)) {
				ImageIO.write(image, "png", new File(qrCodePath + qrName
						+ ".png"));
			}
	 ...
	 * 编码类型 * @param width * 图片宽度 * @param height * 图片高度 * @param hints * 设置参数 * @return
	 */
